...
const columns = [
  { key: "RESEARCH", label: "Research", color: "bg-purple-500", emoji: "" },
  { key: "MOODBOARDING", label: "Moodboard", color: "bg-pink-500", emoji: "" },
  { key: "ANGLES", label: "Angles", color: "bg-indigo-500", emoji: "" },
  { key: "SCRIPTING", label: "Scripting", color: "bg-yellow-500", emoji: "" },
  { key: "APPROVAL_PENDING", label: "Approval", color: "bg-amber-500", emoji: "" },
...
export function UGCVideoBoard({
  ...
  const handleDragOver = useCallback(
    (event: { active: { id: string | number }; over: { id: string | number } | null }) => {
      ...
      const taskIndex = newTasks[activeCol].findIndex((t) => t.id === active.id);
  ...
  return (
    ...
    >
          ...
          return (
            ...
            >
              <div className="mb-3 overflow-hidden rounded-lg">
                <div className={`h-1.5 ${column.color}`} />
                ...
                >
                  <AnimatePresence mode="popLayout">
                    {tasks.map((task) => (
                      ...
                      >
                        <SortableVideoCard task={task} onTaskClick={onTaskClick} />
                      </motion.div>
                    ))}
                  </AnimatePresence>
                </SortableContext>
      ...
      <DragOverlay>
        {activeTask ? (
          <div className="w-64">
            <VideoCard task={activeTask} isOverlay />
          </div>
        ) : null}
      </DragOverlay>
    </DndContext>
